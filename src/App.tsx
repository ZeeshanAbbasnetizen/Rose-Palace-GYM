...
const Navbar = () => {
  ...
  return (
    <nav 
      className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 px-4 sm:px-12 py-4 flex items-center justify-between ${
        isScrolled ? "bg-black/95 backdrop-blur-2xl border-b border-white/5 py-3 shadow-[0_10px_30px_rgba(0,0,0,0.5)]" : "bg-transparent py-8"
      }`}
      id="navbar"
    >
      <div className="flex items-center gap-3">
        <motion.div 
          ...
          className="w-8 h-8 sm:w-10 sm:h-10 bg-[#FF5F05] rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(255,95,5,0.4)]"
        >
          <Dumbbell className="text-black w-4 h-4 sm:w-5 sm:h-5" />
        </motion.div>
        <div className="flex flex-col leading-none">
          <span className="font-heading font-black text-lg sm:text-2xl tracking-tighter uppercase italic">
            Rose<span className="text-[#FF5F05]">Palace</span>
          </span>
          <span className="text-[6px] sm:text-[8px] font-black uppercase tracking-[0.4em] text-white/30 ml-1">Elite Fitness</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <button 
          ...
          className="hidden sm:flex bg-[#FF5F05] text-white px-8 py-3.5 font-black uppercase text-[10px] tracking-widest hover:scale-105 active:scale-95 transition-all items-center gap-2 group shadow-[0_0_20px_rgba(255,95,5,0.3)] relative overflow-hidden" 
          id="nav-join-btn"
        >
        ...
        <Menu 
          className="lg:hidden w-6 h-6 cursor-pointer text-white hover:text-[#FF5F05] transition-colors" 
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            <div className="flex flex-col gap-8">
              {['Home', 'About', 'Experience', 'Gallery', 'Contact'].map((item) => (
                <a 
                  ...
                  className="text-4xl font-display uppercase tracking-tighter hover:text-[#FF5F05] transition-colors"
            ...
            <div className="mt-auto">
              <button 
                onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
                className="w-full bg-[#FF5F05] py-6 font-black uppercase tracking-widest text-lg"
              >
                Join Now
              </button>
            </div>
    ...
    <section id="home" className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-[#0A0A0A]">
      ...
      <div className="relative z-10 text-center px-6 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[100svh] pt-16">
        <motion.div
           ...
           className="mb-6 md:mb-10 cursor-pointer group"
           onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
        >
          <div className="inline-flex items-center gap-3 bg-white/[0.07] backdrop-blur-md px-6 py-2.5 rounded-full border border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.1)] group-hover:bg-white/10 transition-colors">
            <div className="w-2 h-2 bg-[#FF5F05] rounded-full animate-pulse shadow-[0_0_12px_#FF5F05]" />
            <span className="text-white font-black text-[9px] md:text-[10px] tracking-[0.35em] uppercase whitespace-nowrap">Join the Lahore Elite</span>
          </div>
        </motion.div>

        <motion.h1 
          className="font-display text-[16vw] sm:text-[14vw] md:text-[12vw] leading-[0.8] md:leading-[0.75] uppercase tracking-tighter mb-10 md:mb-16 relative flex flex-col items-center justify-center select-none pointer-events-none"
          initial={{ opacity: 0, scale: 1.1 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.2 }}
        >
          <span className="block text-white italic drop-shadow-[0_15px_40px_rgba(0,0,0,0.8)] relative z-20">Rose</span>
          <span className="block text-[#FF5F05] -mt-[0.1em] md:-mt-[0.15em] glow-orange relative z-10 scale-[1.2] md:scale-[1.1] opacity-90 brightness-110">Palace</span>
          
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[6vw] bg-white opacity-[0.015] -rotate-12 pointer-events-none hidden md:block" />
        </motion.h1>

        <motion.div 
          className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-8 w-full max-w-[500px] sm:max-w-none"
        ...
        >
          <button 
            onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
            className="w-full sm:w-auto bg-[#FF5F05] text-white px-10 md:px-16 py-5 md:py-6 font-black uppercase tracking-[0.2em] text-[10px] md:text-[11px] group flex items-center justify-center gap-4 transition-all glow-orange relative overflow-hidden" 
            id="hero-cta-main"
          >
          ...
          <div className="flex flex-col items-center gap-2 w-full sm:w-auto">
            <button 
              ...
              className="w-full sm:w-auto border border-white/10 text-white px-10 md:px-16 py-5 md:py-6 font-black uppercase tracking-[0.2em] text-[10px] md:text-[11px] hover:bg-white hover:text-black transition-all glass-panel" 
              id="hero-tour-main"
            >
              Virtual Tour
            </button>
...
const ServiceCard = ({ title, icon: Icon, description, index }: { title: string, icon: any, description: string, index: number, key?: any }) => {
  ...
  return (
    <motion.div 
      ...
      className="group p-12 bg-white/[0.02] border border-white/5 hover:border-[#FF5F05]/30 transition-all relative overflow-hidden glass-panel"
    >
      ...
      <div className="w-12 h-12 bg-[#FF5F05]/10 rounded-lg flex items-center justify-center mb-6 text-[#FF5F05] group-hover:bg-[#FF5F05] group-hover:text-white transition-all glow-orange relative z-10">
        <Icon className="w-6 h-6" />
      </div>
      <h3 className="font-heading font-black text-xl uppercase mb-3 tracking-tighter italic relative z-10">{title}</h3>
      <p className="text-white/30 text-xs font-medium leading-relaxed uppercase tracking-widest relative z-10">{description}</p>
      
      <div className="mt-8 flex items-center gap-2 text-[#FF5F05] text-[10px] font-black uppercase tracking-[0.2em] cursor-pointer group-hover:gap-4 transition-all relative z-10">
        Experience Details <ArrowRight className="w-4 h-4" />
      </div>
    </motion.div>
  );
};
    ...
    <section id="services" className="py-32 px-6 bg-[#0A0A0A]">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col mb-24 text-center items-center">
            <span className="text-[#FF5F05] font-black uppercase tracking-[0.4em] text-[10px] mb-4">Daily Inspiration</span>
            <h2 className="font-heading font-black text-4xl md:text-7xl uppercase tracking-tighter mb-6 italic leading-none">
              Complimentary <span className="text-white/20 not-italic outline-text">Services</span>
            </h2>
            <p className="max-w-2xl text-white/30 font-black uppercase tracking-widest text-[10px] mt-4">Elevating your experience with world-class amenities at no extra cost.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
          {services.map((service, i) => (
            <motion.div 
              ...
              className="group relative h-[450px] overflow-hidden rounded-3xl border border-white/5"
            >
              ...
              <div className="absolute inset-0 p-10 flex flex-col justify-end">
                <div className="w-12 h-12 bg-[#FF5F05] rounded-xl flex items-center justify-center mb-6 shadow-lg glow-orange">
                  <service.icon className="w-6 h-6 text-white" />
                </div>
                <h3 className="font-heading font-black text-2xl uppercase italic mb-4 tracking-tighter transition-colors group-hover:text-[#FF5F05]">{service.title}</h3>
                <p className="text-white/40 text-[11px] font-black uppercase tracking-widest leading-relaxed">{service.description}</p>
              </div>
            </motion.div>
          ))}
        </div>

        <div className="bg-white/[0.03] border border-white/5 rounded-[40px] p-10 md:p-16 relative overflow-hidden">
           <div className="absolute top-0 right-0 p-10 opacity-5">
              <Zap className="w-40 h-40 text-white" />
           </div>
           <h3 className="text-2xl font-heading font-black uppercase italic tracking-tighter mb-12 text-center flex items-center justify-center gap-4">
             <div className="h-[1px] w-12 bg-[#FF5F05]" />
             We Offer (Complimentary)
             <div className="h-[1px] w-12 bg-[#FF5F05]" />
           </h3>
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-12">
              {complimentaryItems.map((item, i) => (
                <div key={i} className="flex items-center gap-3 text-white/50 group border-b border-white/5 pb-4">
                  <CheckCircle2 className="w-4 h-4 text-[#FF5F05] group-hover:scale-125 transition-transform" />
                  <span className="text-[10px] font-black uppercase tracking-widest group-hover:text-white transition-colors">{item}</span>
                </div>
              ))}
           </div>
        </div>
...
const MapSection = () => {
  ...
  return (
    <section className="h-[450px] w-full bg-black relative border-y border-white/5 group cursor-pointer" onClick={openMap}>
      <iframe 
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 flex flex-col items-center gap-4 text-center">
         <div className="bg-[#FF5F05] w-14 h-14 rounded-full flex items-center justify-center animate-bounce shadow-[0_0_30px_#FF5F05]">
           <MapPin className="text-black w-6 h-6" />
         </div>
         <div className="bg-black/90 backdrop-blur-xl border border-white/10 p-4 rounded-xl">
           <h4 className="text-[#FF5F05] font-black text-xs uppercase tracking-widest italic group-hover:tracking-[0.2em] transition-all">Get Directions (Gulberg-II)</h4>
           <div className="flex items-center gap-2 justify-center mt-1 text-white/30 text-[8px] font-black">
    ...
    <section id="contact" className="py-32 px-6 bg-[#0A0A0A]">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-32">
          <div>
            <span className="text-[#FF5F05] font-black uppercase tracking-[0.4em] text-[10px] block mb-4">Get in Touch</span>
            <h2 className="text-5xl md:text-7xl font-heading font-black uppercase tracking-tighter italic mb-8 leading-none">
              Better yet, <br /> see us in <span className="text-[#FF5F05]">person!</span>
            </h2>
            <p className="text-white/40 font-black uppercase tracking-widest text-[11px] mb-16 leading-relaxed max-w-md">
              We are open 24/7 so feel free to visit our state-of-the-art facility in Gulberg-II.
            </p>
            
            <div className="space-y-12">
              <div className="flex items-start gap-6">
                <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center shrink-0 border border-white/10 group-hover:border-[#FF5F05] transition-colors">
                  <MapPin className="w-5 h-5 text-[#FF5F05]" />
                </div>
                <div>
                  <h4 className="text-[#FF5F05] font-black uppercase tracking-widest text-[10px] mb-2">Location</h4>
                  <p className="text-white/60 text-sm font-bold uppercase tracking-widest leading-loose">Rose Palace Gym (Gulberg), Gurumangat Road, Gulberg 2, Lahore, Pakistan</p>
                </div>
              </div>
              <div className="flex items-start gap-6">
                <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center shrink-0 border border-white/10 group-hover:border-[#FF5F05] transition-colors">
                  <Phone className="w-5 h-5 text-[#FF5F05]" />
                </div>
                <div>
                  <h4 className="text-[#FF5F05] font-black uppercase tracking-widest text-[10px] mb-2">Call Now</h4>
                  <p className="text-white/60 text-2xl font-heading font-black italic">+92 335 433 7172</p>
                </div>
              </div>
              <div className="flex items-start gap-6">
                <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center shrink-0 border border-white/10 group-hover:border-[#FF5F05] transition-colors">
                  <Mail className="w-5 h-5 text-[#FF5F05]" />
                </div>
                <div>
                  <h4 className="text-[#FF5F05] font-black uppercase tracking-widest text-[10px] mb-2">Email</h4>
                  <p className="text-white/60 font-bold uppercase tracking-widest">info@rosepalacegym.com</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            <div className="absolute inset-0 bg-[#FF5F05]/5 blur-[80px]" />
            <div className="relative bg-white/[0.02] border border-white/10 p-10 md:p-16 rounded-[40px] glass-panel backdrop-blur-xl">
               <h3 className="text-4xl font-heading font-black uppercase tracking-tighter italic mb-10">Join Now <span className="text-[#FF5F05]">👇</span></h3>
               
               <form className="space-y-8" onSubmit={handleSubmit}>
                 <div className="space-y-2">
                   <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-4">Name*</label>
                   <input 
                     type="text" 
                     required
                     value={formData.name}
                     ...
                     className="w-full bg-black/50 border border-white/5 rounded-2xl px-6 py-5 text-sm font-bold placeholder:text-white/10 outline-none focus:border-[#FF5F05] transition-colors" 
                   />
                 </div>
                 <div className="space-y-2">
                   <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-4">Mobile*</label>
                   <input 
                     type="text" 
                     required
                 ...
                 {status.type && (
                   <div className={`p-4 rounded-xl text-xs font-black uppercase tracking-widest ${status.type === 'success' ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'}`}>
                     {status.message}
                   </div>
                 )}
                 ...
                 <button 
                   ...
                   className="w-full bg-[#FF5F05] text-white py-6 rounded-2xl font-black uppercase tracking-[0.3em] text-xs glow-orange hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_20px_40px_rgba(255,95,5,0.2)] disabled:opacity-50 disabled:cursor-not-allowed"
                 >
...
const Marquee = () => {
  return (
    <div className="py-12 bg-[#FF5F05] overflow-hidden whitespace-nowrap -rotate-1 relative z-20 shadow-xl">
      <motion.div 
        animate={{ x: [0, -1000] }}
        transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
        className="flex gap-20 items-center uppercase font-heading font-black text-4xl md:text-6xl text-white select-none"
      >
        <span>Yoga & Zumba</span>
        <span className="outline-text" style={{ WebkitTextStroke: '2px white', color: 'transparent' }}>Personal Training</span>
        <span>24/7 Access</span>
        <span>Gulberg II Lahore</span>
        <span className="outline-text" style={{ WebkitTextStroke: '2px white', color: 'transparent' }}>Spinning Studio</span>
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 p-8">
        <h4 className="text-3xl font-heading font-black uppercase tracking-tighter italic">{title}</h4>
        <div className="flex items-center gap-2 mt-2 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
          <span className="text-[10px] font-black uppercase tracking-widest text-[#FF5F05]">View Facility</span>
          <ChevronRight className="w-4 h-4 text-[#FF5F05]" />
        </div>
      </div>
    </motion.div>
  );
...
const Facilities = () => {
  ...
  return (
    <section className="py-32 px-6 bg-black">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16">
          <span className="text-[#FF5F05] font-black uppercase tracking-[0.4em] text-[10px] block mb-4">State of the Art</span>
          <h2 className="text-5xl md:text-7xl font-heading font-black uppercase tracking-tighter leading-none italic mb-8">
...
const VideoSection = () => {
  return (
    <section id="video-section" className="py-32 px-6 bg-black overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col items-center text-center">
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          whileInView={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1 }}
          className="relative w-full max-w-4xl aspect-video rounded-[40px] overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(255,95,5,0.1)] mb-16 bg-[#111]"
        >
          <iframe 
          ...
          ></iframe>
          <div className="absolute inset-0 pointer-events-none border-[1px] border-white/5 rounded-[40px]" />
        </motion.div>
        ...
        >
          <div className="relative mb-12 flex flex-col items-center">
            <h2 className="text-[#EE0000] font-heading font-black text-6xl md:text-[10vw] uppercase tracking-tighter italic z-20 relative drop-shadow-[0_20px_50px_rgba(0,0,0,1)]">
              ROSE
            </h2>
            <h2 className="text-[#EE0000] font-heading font-black text-6xl md:text-[10vw] uppercase tracking-tighter italic -mt-[0.1em] md:-mt-[0.2em] opacity-40 z-10 scale-[1.1]">
              PALACE
            </h2>
          </div>
          <p className="max-w-3xl mx-auto text-white/50 font-black uppercase tracking-[0.2em] text-xs md:text-sm leading-loose">
...
const Footer = () => {
  return (
    <footer id="contact" className="bg-[#0A0A0A] border-t border-white/5 pt-32 pb-10 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-20">
          <div className="col-span-1 md:col-span-1">
            <div className="flex items-center gap-2 mb-8">
                <div className="w-8 h-8 bg-[#FF5F05] rounded-full flex items-center justify-center">
                <Dumbbell className="text-black w-5 h-5" />
                </div>
                <span className="font-heading font-black text-xl tracking-tighter uppercase italic">
                Rose<span className="text-[#FF5F05]">Palace</span>
                </span>
            </div>
            <p className="text-white/30 text-xs mb-8 leading-relaxed font-medium uppercase tracking-widest">
              55-N, Gurumangat Road, <br />
              Gulberg-II, Lahore, Pakistan
            </p>
            <div className="flex gap-4">
              <a href="https://www.instagram.com/rosepalacegym/" target="_blank" rel="noopener noreferrer">
                <Instagram className="w-5 h-5 text-white/40 hover:text-[#FF5F05] cursor-pointer transition-all" />
              </a>
              <a href="https://www.facebook.com/rosepalacegym/" target="_blank" rel="noopener noreferrer">
                <Facebook className="w-5 h-5 text-white/40 hover:text-[#FF5F05] cursor-pointer transition-all" />
              </a>
              <Twitter className="w-5 h-5 text-white/40 hover:text-[#FF5F05] cursor-pointer transition-all" />
            </div>
          </div>
          
          <div>
            <h4 className="font-black uppercase tracking-widest text-xs mb-8 text-[#FF5F05]">Quick Links</h4>
